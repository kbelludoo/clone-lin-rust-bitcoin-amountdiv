/*
 * Independent C11 oracle for rust-bitcoin Amount::checked_div / checked_rem
 * and div_by_weight_floor (CC0-1.0 algorithm).
 *
 * Weight floor: (sats * 4_000_000) / wu uses unsigned __int128 AND a portable
 * 32-bit limb clone matching lin_rust_bitcoin_amountdiv.lin.
 *
 * Option None is encoded as -1 (Amount::ZERO is 0).
...
static int64_t checked_div(int64_t sat, int64_t rhs) {
    if (from_sat(sat) < 0) return -1;
    if (rhs <= 0) return -1;
    return sat / rhs;
}

static int64_t checked_rem(int64_t sat, int64_t rhs) {
    if (from_sat(sat) < 0) return -1;
    if (rhs <= 0) return -1;
    return sat - (sat / rhs) * rhs;
}
...
static int selftest(void) {
    ...
    if (checked_div(100, 3) != 33) return 4;
    if (checked_rem(100, 3) != 1) return 5;
    if (checked_div(100, 0) != -1) return 6;
    if (wfloor(10, 200, 0) != 200000) return 7;
    if (wfloor(10, 200, 1) != 200000) return 8;
    if (wrem(10, 300, 0) != 100) return 9;
    if (wrem(10, 300, 1) != 100) return 10;
    if (wfloor(10, 300, 0) != 133333) return 11;
    if (wfloor((int64_t)MAX_MONEY, 1, 0) != -1) return 12;
    if (wfloor((int64_t)MAX_MONEY, 1, 1) != -1) return 13;
    muldiv_limbs_qr(100, 200, 50, &q, &r, &ov);
    if (q != 400 || r != 0 || ov) return 14;
    if (muldiv_i128(100, 200, 50, 0) != 400) return 15;
    printf("PASS\n");
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    const char *cmd;
    if (argc < 2) {
        fprintf(stderr, "usage: %s selftest | from_sat N | div A B | rem A B | wfloor A W | wrem A W | muldiv A B D [--limbs]\n", argv[0]);
        return 2;
    }
    for (i = 2; i < argc; i++) {
    ...
    if (strcmp(cmd, "div") == 0 && argc >= 4) {
        printf("%" PRId64 "\n", checked_div(atoll(argv[2]), atoll(argv[3])));
        return 0;
    }
    if (strcmp(cmd, "rem") == 0 && argc >= 4) {
        printf("%" PRId64 "\n", checked_rem(atoll(argv[2]), atoll(argv[3])));
        return 0;
    }
